...
var source, j, i;
	...
	{
		cdns.push("<br>", GetMessage("Process", "llinatge")," ");
...
var cdns=[], node, i_nod;
	...
	for (i_nod=0; i_nod<params.nodes.length; i_nod++)
	{
		...
		{
			cdns.push(GetMessage("ResultingDataset", "llinatge"), ": ", DonaCadena(node.capa.desc) ? DonaCadena(node.capa.desc): node.capa.nom);
		...
		{
			cdns.push(GetMessage("Source"),": ", node.font.reference);
		}
		else if(node.group=="proces")
		{
			cdns.push(GetMessage("Process", "llinatge"),": ", DonaCadena(node.proces.purpose));
		}
		else if(node.group=="agent")
		{
			if(DonaCadena(node.responsibleParty.role))
				...
				cdns.push(GetMessage("Agent"), ": ");
			cdns.push(GetPartyName(node.responsibleParty.party));
		}
		else if(node.group=="executable")
		{
			cdns.push(GetMessage("Executable"), ": ", node.executable.reference);
			if (node.executable.compilationDate)
				cdns.push("<br>", GetMessage("CompilationDate"), ": ", node.executable.compilationDate);
			if (node.executable.algorithm)
			{
				cdns.push("<br>", GetMessage("Algorithm"), ": ", DonaCadena(node.executable.algorithm.name));
				if (node.executable.algorithm.functionality)
					cdns.push("<br>", GetMessage("Functionality"), ": ", DonaCadena(node.executable.algorithm.functionality));
			}
		}
		else if(node.group=="algorisme")
		{
			if (node.algorithm)
			{
				cdns.push("<br>", GetMessage("Algorithm"), ": ", DonaCadena(node.algorithm.name));
				if (node.algorithm.functionality)
					cdns.push("<br>", GetMessage("Functionality"), ": ", DonaCadena(node.algorithm.functionality));
			}
		}
		else if(node.group=="funcionalitat")
		{
			if (node.functionality)
				cdns.push("<br>", GetMessage("Functionality"), ": ", DonaCadena(node.functionality));
		}
	}
	return cdns.join("");
}
...
var cdns=[];
	...
	if(!node.lineage)	// Si és el llinatge, vol dir que ho tinc tot agrupat
	{
		...
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"AgrupaAmbAnteriorNodeLlinatge(", node, ");TancaContextMenuCapa();\">",
				GetMessage("GroupWithPrevious", "llinatge"), "</a><br>");
	}
...
var cdns=[],capa;
	...
	cdns.push(	"<legend>",GetMessage("Layers"), "</legend>");
	for(var i=0;i<LListaCapesGraphsMM.length; i++)
	{
	...
	if(LListaCapesGraphsMM.length>0)
		cdns.push("<font size=1><a href=\"javascript:void(0);\" onClick=\"EsborraGrafLlinatgeICapes();\">", GetMessage("DeleteAll", "consola"),"</a></font>");
	return cdns.join("");
}
...
{
var cdns=[];
	cdns.push("<legend>",GetMessage("VisibleElements", "llinatge"), "</legend>",
		...
		" <label for=\"eLl_fontsIntermitges\">", GetMessage("InternalSources", "llinatge"), "</label>",
		...
		" <label for=\"eLl_fontsFulles\">", GetMessage("LeafSources", "llinatge"), "</label>",
		...
		" <label for=\"eLl_agents\">", GetMessage("Agents"), "</label><br>",
		...
		" <label for=\"eLl_passos\">", GetMessage("ProcessSteps", "llinatge"), "</label>",
		...
		" <label for=\"eLl_eines\">", GetMessage("ProcessingTools", "llinatge"), "</label>",
		...
		" <label for=\"eLl_algorismes\">", GetMessage("Algorithms"), "</label>",
		...
		" <label for=\"eLl_funcions\">", GetMessage("Functionalities"), "</label>");

	if(LListaCapesGraphsMM.length>1)
	{
		cdns.push("<fieldset id=\"conjuntsLlinatge\">",
			"<legend>",GetMessage("MoreOneDataset", "llinatge") + ", " + GetMessage("show") + ":", "</legend>",
			...
			" <label for=\"union\">", GetMessage("TheUnion", "llinatge"), "</label>",
			...
			" <label for=\"nonUnion\">", GetMessage("DatasetsIndependentsGraphs", "llinatge"), "</label><br>",
...
var valor=[], i, p;
	if(node.capa)
	{
		valor.push(GetMessage("Source"), ": ", DonaCadena(node.capa.desc) ? DonaCadena(node.capa.desc): node.capa.nom);
	...
	if(node.process)
	{
		valor.push(GetMessage("Process", "llinatge"),": ", DonaLabelPerProces(node.process, null));
	...
	{
		valor.push(GetMessage("Source"), ": ", TreuAdreca(node.source.reference));
		llista_nodes.push({tipus: "font", etiqueta: valor.join("")});
	...
	if(node.reponsibleParty)
	{
		...
		if (!name || name=="")
			return llista_nodes;
		valor.push(GetMessage("Agent"), ": ",  name.indexOf(' ')==-1 ? name : name.substring(0, name.indexOf(' ')));
		llista_nodes.push({tipus: "agent", etiqueta: valor.join("")});
		return llista_nodes;
	}
	if(node.exe)
	{
		valor.push(GetMessage("Executable"), ": ",  TreuAdreca(node.exe.reference));
	...
	if(node.algorithm)
	{
		valor.push(GetMessage("Algorithm"), ": ",  DonaCadena(node.algorithm.name));
	...
	if(node.functionality)
	{
		valor.push(GetMessage("Functionality"), ": ",  DonaCadena(node.functionality));
		llista_nodes.push({tipus: "algorisme", etiqueta: valor.join("")});
		return llista_nodes;
	}
	return llista_nodes;
...
var cdns=[], llista_nodes=[], i;
	...
	cdns.push("<legend>",GetMessage("SimpleQuery", "llinatge"), "</legend>",
			  GetMessage("StartNode", "llinatge"), " <select name=\"sQLl_startNode\">");
	for(i=0; i<llista_nodes.length; i++)
		cdns.push("<option value=\"",i,"\">", llista_nodes[i].etiqueta);
	cdns.push("</select><br>",GetMessage("EndNode", "llinatge"), " <select name=\"sQLl_endNode\">");
	for(i=0; i<llista_nodes.length; i++)
		cdns.push("<option value=\"",i,"\">", llista_nodes[i].etiqueta);
	cdns.push("</select>");
	return cdns.join("");
...
var i_sel, tipus_node,sel_a_omplir;
	...
	if(tipus_node=="agent")
	{
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("role"), value:"role", selected: true});
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("name"), value:"name", selected: false});
	}
	...
	{
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("purpose"), value:"purpose", selected: true});
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("date"), value:"date", selected: false});
	}
	else if(tipus_node=="exe")
	{
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("name"), value:"name", selected: true});
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("reference"), value:"reference", selected: false});
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("compilationDate"), value:"compilationDate", selected: false});
	}
	else if(tipus_node=="algorisme")
	{
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("name"), value:"name", selected: true});
		AfegeixOptionASelect(sel_a_omplir, {text: GetMessage("Functionality"), value:"functionality", selected: false});
	}
...
var cdns=[];
	cdns.push("<legend>",GetMessage("ComplexQuery", "llinatge"), "</legend>",
      ...
      GetMessage("Type"),
			...
			"<option value=\"proces\">", GetMessage("Process", "llinatge"),
			"<option value=\"exe\">", GetMessage("Executable"),
			"<option value=\"algorisme\">", GetMessage("Algorithm"),
			"<option value=\"funcio\">", GetMessage("Functionality"),
			"</select>",
			GetMessage("Attribute") + ": ",
			"<select name=\"cQLl_startAttributeNode\">",
			"<option value=\"role\" selected>", GetMessage("role"),"</option>",
