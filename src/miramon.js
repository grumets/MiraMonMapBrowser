...
function CanviaIdioma(s)
{
	...
	elem=getFinestraLayer(window, "lbScope");
	if(isLayer(elem) && isLayerVisible(elem))
		OmpleFinestraLogBookAmbScope();
...
function TancaFinestraLayer(nom_finestra)
{
	...
	else if (nom_finestra=="lbScope")
		TancaFinestraLogBookAmbScope();	
...
var cdns=[], tipus, plantilla, i_estil2=-1, capa=ParamCtrl.capa[i];
	...
	if(ParamCtrl.ImatgeSituacio[ParamInternCtrl.ISituacio].EnvTotal.CRS.indexOf("EPSG:")!=-1 ||
		...
		url_crs=DonaURLOpengisDeEPSG(ParamCtrl.ImatgeSituacio[ParamInternCtrl.ISituacio].EnvTotal.CRS);
	if(!url_crs)
		url_crs=ParamCtrl.ImatgeSituacio[ParamInternCtrl.ISituacio].EnvTotal.CRS;
	if (tipus=="TipusOAPI_Maps")
	{
		cdns.push("crs=", url_crs, // crs de la imatge
	...
	if(tipus=="TipusOAPI_Maps")
	{
		cdns.push("&width=" , ncol , "&height=" , nfil,
				"&f=" , (capa.FormatImatge == "image/heif" ? "hej2" : (capa.FormatImatge == "image/png" ? "png" : (capa.FormatImatge == "image/jpeg" ? "jpeg" : capa.FormatImatge))),
	...
	else
	{
		...
		if (capa.estil && capa.estil.length)
		{
			...
			if (capa.estil[i_estil2].nom)
			{
				...
				{
					cdns.push(capa.estil[i_estil2].nom);
					if (pot_semitrans && capa.FormatImatge!="image/jpeg" && capa.visible=="semitransparent" && ParamCtrl.TransparenciaDesDeServidor)
						cdns.push(":SEMITRANSPARENT");
				}
			}
...
var i, j, l, titolFinestra, div=document.getElementById(ParamCtrl.containerName);
	...
	createFinestraLayer(window, "lbScope", GetMessageJSON("LBwithScope", "barra"), boto_tancar, 297, 298, 400, 300, "NwCR", {scroll: "ara_no", visible: false, ev: null, resizable:true}, null);
