		...
		*/
		...
		else if (estil.TipusObj == "P")
		{
				...
				arrayKeyes.forEach((item, i) => {
					...
					if (forma.paleta && forma.paleta.colors && forma.paleta.colors.length > 0)
					{
						...
						else if (forma.paleta.colors[0].indexOf("#") == -1)
						{
							...
							const hexColor = "#" + ((1 << 24) + (arrayColorRGBA[0] << 16) + (arrayColorRGBA[1] << 8) + arrayColorRGBA[2]).toString(16).slice(1);

							arrayColorsSelectors.push({color:hexColor, descr:objKey});
							if (arrayColorRGBA.length == 4)
							{
								arrayAlphaSelectors.push({alpha:arrayColorRGBA[arrayColorRGBA.length-1].parseInt, descr:objKey});
							}
						}
					}
				});
			...
			if(arrayAlphaSelectors && arrayAlphaSelectors.length)
			{
				...
				for (var indexTranspSel = 0, itemsTransSelLength = arrayAlphaSelectors.length; indexTranspSel < itemsTransSelLength; indexThickSel++)
				{
					const labelString = arrayAlphaSelectors[indexColorSel].descr.stringValue.replace(/([A-Z]+)/g, " $1").replace(/([A-Z][a-z])/g, " $1");
					cdns.push("<tr><td><input type=\"text\" name=\"Gruixos\" size=\"2\" id=\"edita-estil-transparencia-" + indexTranspSel + "\" value=\"" + arrayAlphaSelectors[indexTranspSel].alpha + "\"></td><label class=\"Verdana11px\" for=\"edita-estil-transparencia-" + indexTranspSel + "\">", labelString, "</label></td></tr></tr>");
				}
				cdns.push("</table></fieldset>");
			}
		}
