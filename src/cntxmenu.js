/*
    ...
    (joan maso at uab cat) i N�ria Juli� (n julia at creaf uab cat)
...
var capa=ParamCtrl.capa[i_capa], alguna_opcio=false;
	...
	if (ParamCtrl.capa.length>NumeroDeCapesEspecials(-1))
	{
		cdns.push("<b><font color=\"#888888\">",
			  DonaCadenaLang({"cat":"Moure la capa", "spa":"Mover la capa", "eng":"Move layer", "fre":"D�placer la couche"}), "</b>");
	...
	if (capa.metadades && capa.metadades.standard && DonaCadena(capa.metadades))
	{
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraFitxerMetadades(", i_capa, ", -1);TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Metadades", "spa":"Metadatos", "eng":"Metadata", "fre":"M�tadonn�es"}), "</a><br>");
	}
	if (/*(capa.tipus=="TipusWMS" && capa.FormatImatge=="application/x-img") ||*/ capa.tipus=="TipusWFS" || capa.tipus=="TipusSOS")
	{
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraCalculaQualitatCapa(",i_capa,", -1);TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Calcula la qualitat", "spa":"Calcula la calidad", "eng":"Compute the quality", "fre":"Calculer la qualit�"}), "</a><br>");
	}
	if (capa.metadades && capa.metadades.quality)
	{
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraMostraQualitatCapa(", i_capa,", -1);TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Qualitat", "spa":"Calidad", "eng":"Quality", "fre":"Qualit�"}), "</a><br>");
	}
	...
	{*/
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraFeedbackCapa(", i_capa,", -1);TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Valoracions", "spa":"Valoraciones", "eng":"Feedback", "fre":"r�troaction"}), "</a><br>");
	//}
	cdns.push("<hr>");
	...
	if (capa.FormatImatge=="application/x-img" && capa.estil && capa.estil.length && capa.estil[capa.i_estil].histograma)
	{
		...
		if (estil.component && estil.component[0].representacio && estil.component[0].representacio.tipus=="matriuConfusio")
			cdns.push(DonaCadenaLang({"cat":"Matriu de confusi�", "spa":"Matriz de confusi�n", "eng":"Confusion matrix", "fre":"Matrice de confusion"}));
		else if (estil.categories && estil.atributs)
			cdns.push(DonaCadenaLang({"cat":"Gr�fic circular", "spa":"Gr�fico circular", "eng":"Pie chart", "fre":"Diagramme � secteurs"}));
		else
	...
	if (capa.valors && capa.valors.length>2)
	{
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraCombinacioRGB(", i_capa, ");TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Combinaci� RGB", "spa":"Combinaci�n RGB", "eng":"RGB combination", "fre":"Combinaison RVB"}), "</a><br>");
	}
	if (capa.FormatImatge=="application/x-img" || capa.model==model_vector)
	{
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraSeleccioCondicional(", i_capa, ");TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Selecci�", "spa":"Selecci�n", "eng":"Selection", "fre":"S�lection"}), "</a><br>");
	}
	if (capa.FormatImatge=="application/x-img")
	{
		cdns.push("<a class=\"unmenu\" href=\"javascript:void(0);\" onClick=\"ObreFinestraReclassificaCapa(",i_capa,");TancaContextMenuCapa();\">",
				DonaCadenaLang({"cat":"Reclassificaci�", "spa":"Reclasificaci�n", "eng":"Reclassification", "fre":"Reclassement"}), "</a><br>");
	}
	if (cdns.length==0)
		return false;
...
var minim, maxim;
	...
	if (ParamCtrl.LlegendaAmagaSegonsEscala==true && !EsCapaDinsRangDEscalesVisibles(ParamCtrl.capa[k]))
		   alert(DonaCadenaLang({"cat":"La nova capa afegida, \'"+ParamCtrl.capa[k].nom+"\' no �s visible al nivell de zoom actual del navegador",
							 "spa":"La nueva capa a�adida, \'"+ParamCtrl.capa[k].nom+"\' no es visible al nivel de zoom actual del navegador",
							 "eng":"The new layer added, \'"+ParamCtrl.capa[k].nom+"\' is not visible in the current zoom level of the browser",
							 "fre":"La nouvelle couche ajout�e, \'"+ParamCtrl.capa[k].nom+"\' n'est pas visible au niveau du zoom actuel du navigateur"}));
}
...
//i_capa_ini �s la capa inicial per fer el canvi d'indexos
//i_capa_fi_per_sota �s la capa fi (no incluent ella mateixa) on cal fer el canvi d'indexos. Opcional; si no s'especifica, val i_capa_ini+1
...
var i, old_value, old_up_value, new_value, desc_value, inici, final;
	...
	else
	{
		if(-1!=old_value.search(/["|']/i))
		{
			...
			{
				alert(DonaCadenaLang({"cat": "Valor a canviar incorrecte a la l�nia",
...
var minim, maxim, factor_k, factorpixel;
	...
	if(trobat || sistema_ref_comu)
	{
		...
		if(i<node.childNodes.length)  //vol dir que aquesta capa t� name
		{
			...
			{
				if(valors_temps.search("/")==-1)  //Si �s un interval (!=-1) de moment no li dono suport �$�
				{
...
var root, cadena, node, node2, i, cdns=[];
	...
	if(!doc)
	{
		alert(DonaCadenaLang({"cat":"No s'ha obtingut cap resposta v�lida del servidor sol�licitat",
						  "spa":"No se ha obtenido ninguna respuesta v�lida del servidor solicitado",
						  "eng":"Cannot obtain any valid response of server",
						  "fre":"Aucune r�ponse valide a �t� obtenue depuis le serveur sollicit�"}));
		return;
	}
	...
	if(servidorGC.layer.length>0)
	{
		...
		cdns.push(" /><br><br><hr><br><div class=\"layerselectorcapesafegir\">",
				  ...
				  DonaCadenaLang({"cat":"Seleccionar totes les capes", "spa":"Seleccionar todas las capas", "eng":"Select all layers", "fre":"S�lectionner toutes les couches"}), "<br><br><table class=\"Verdana11px\">");
	...
	{
		alert(DonaCadenaLang({"cat":"Aquest servidor no t� cap capa disponible en el sistema de refer�ncia actual del navegador",
...
var request;
	...
	if(!servidor || servidor=="")
	{
		...
		alert(DonaCadenaLang({"cat":"Cal indicar una adre�a v�lida",
...
var i, j;
	...
	while( i<LlistaServOWS.length && categoria_sel!=DonaCadena(LlistaServOWS[i].categoria.desc).toLowerCase())
		i++;
	j=0;
	form.llista_serveis_OWS.options[j]=new Option(DonaCadenaLang({"cat":"--Seleccciona'n un de la llista--", "spa":"--Escoja uno de la lista--","eng": "--Choose one from list--", "fre":"--S�lectionnez un objet de la liste--"}), "");
	j++;
	while(i<LlistaServOWS.length && categoria_sel==DonaCadena(LlistaServOWS[i].categoria.desc).toLowerCase())
	{
...
{
var elem=ObreFinestra(window, "info", DonaCadenaLang({"cat":"per mostrar informaci� o ajuda",
...
var cdns=[], i, capa;
	...
	cdns.push("<br><fieldset><legend>",
			  DonaCadenaLang({"cat":"Afegeix capa reclassificada com un nou estil", "spa":"A�ada capa reclasificada como un nuevo estilo",
							"eng":"Add reclassified layer as a new style", "fre":"Ajouter une couche reclass�e en tant que nouveau style"}),
			  "</legend>",
			  "<fieldset><legend>",
			  DonaCadenaLang({"cat":"Capa a reclassificar ", "spa":"Capa a reclasificar ", "eng":"Layer to reclassify", "fre":"Couche � reclassifier"}),
			  "</legend>",
	...
	cdns.push(DonaCadenaLang({"cat":"F�rmula de reclassificaci�", "spa":"F�rmula de reclasificaci�n:", "eng":"Reclassifying expression", "fre":"Formule de reclassement"}),
				...
				"<hr>",
				DonaCadenaLang({"cat":"El resultat de la reclassificaci� ser� afegit com a un estil nou de nom", "spa":"El resultado de la reclasssificaci�n ser� a�adido como un estilo nuevo de nombre",
								"eng":"The result of the reclassification will be added as a new style with name", "fre":"Le r�sultat du reclassement sera ajout� en tant que nouveau style avec le nom"}),
				...
				"\" /><br/>",
				DonaCadenaLang({"cat":"a la capa", "spa":"a la capa", "eng":"to the layer", "fre":"� la couche"}),
				" \"", DonaCadena(capa.DescLlegenda), "\"<br/>",
				"<input type=\"button\" class=\"Verdana11px\" value=\"",
				DonaCadenaLang({"cat":"Afegir", "spa":"A�adir", "eng":"Add", "fre":"Ajouter"}),
...
var cdns=[], i, capa, hi_ha_rasters=0, hi_ha_raster_categ=0;
	...
	if (hi_ha_rasters)
	{
		cdns.push("<br><fieldset><legend>",
			  ...
			  "</legend>",
			  "<fieldset><legend>",
			  DonaCadenaLang({"cat":"Capa per a la f�rmula", "spa":"Capa para la f�rmula", "eng":"Layer for the expression", "fre":"Couche pour l'expression"}),
			  "</legend>");
		...
		cdns.push("<input type=\"button\" class=\"Verdana11px\" value=\"",
		     	DonaCadenaLang({"cat":"Escriu a la f�rmula", "spa":"Escribe en f�rmula", "eng":"Write in expression", "fre":"Ecrire � la formule"}),
		        "\" onClick='EscriuCapaALaFormulaAfegeixCapa();' /></fieldset>");
		//Caixa multil�nia per a la formula.
		cdns.push(DonaCadenaLang({"cat":"F�rmula", "spa":"F�rmula:", "eng":"Expression", "fre":"Formule"}),
	...
	{
		cdns.push("<br><fieldset><legend>",
			  DonaCadenaLang({"cat":"Afegeix capa combinada a partir de dues capes existents", "spa":"A�ada capa combinada a partir de dues capas existentes", "eng":"Add layer combined from two existing layers", "fre":"Rajouter couche combin� � partir de deux couches existantes"}),
			  "</legend>",
			  "<fieldset><legend>",
...
var cdns=[], i;
	...
	cdns.push("<div id=\"LayerAfegeixCapaServidor\" class=\"Verdana11px\" style=\"position:absolute; left:10px; top:10px;\">",
			...
			"</legend>",
			DonaCadenaLang({"cat":"Especifica l'adre�a URL del servidor", "spa":"Especifique la direcci�n URL del servidor", "eng":"Specify the server URL", "fre":"Sp�cifiez l'adresse URL du serveur"}),
	...
	{
		cdns.push("<br><br>",
			DonaCadenaLang({"cat":"o Seleccciona'n un de la llista de serveis", "spa":"o Escoja uno de la lista de servicios", "eng":"or Choose one from service list", "fre":"ou s�lectionnez un des services de la liste"}),
			"<br>",
		...
		i=0;
		cdns.push("<option value=\"\">",  DonaCadenaLang({"cat":"--Seleccciona'n un de la llista--", "spa":"--Escoja uno de la lista--","eng": "--Choose one from list--", "fre":"--S�lectionnez un des objets de la liste--"}));
...
{
var elem=ObreFinestra(window, "calculadoraCapa", DonaCadenaLang({"cat":"per fer c�lculs de capes",
...
var estil=capa.estil[i_estil];
	...
	if (estil.categories && estil.categories.length && estil.atributs)
	{
		...
		cdns.push("<input type=\"button\" class=\"Verdana11px\" value=\"",
		     	DonaCadenaLang({"cat":"Escriu valor a la f�rmula", "spa":"Escribe valor en f�rmula", "eng":"Write value in expression", "fre":"�crire une valeur dans l'expression"}),
	...
	else
	{
		if (estil.component[0].estiramentPaleta && typeof estil.component[0].estiramentPaleta.valorMaxim!=="undefined" && typeof estil.component[0].estiramentPaleta.valorMinim!=="undefined")
			cdns.push(DonaCadenaLang({"cat":"Interval de valors recomenats", "spa":"Intervaluo de valores recomendados", "eng":"Interval of recommended values", "fre":"Intervalle des valeurs recommand�es"}), ": [", estil.component[0].estiramentPaleta.valorMinim, ",", estil.component[0].estiramentPaleta.valorMaxim, "]");
	}
	//cdns.push("</div>");
	return cdns.join("");
}
...
var estil_o_atrib;
	...
	cdns.push("<div id=\"div-", prefix_id, "-operador-",i_condicio,"\" ><label for=\"", prefix_id, "-operador-",i_condicio, "\">",
			  DonaCadenaLang({"cat":"Operador", "spa":"Operador", "eng":"Operator", "fre":"Op�rateur"}), ":</label>",
	...
	else
	{
		...
		else
		{
			...
			if (estil_o_atrib.component[0].estiramentPaleta && typeof estil_o_atrib.component[0].estiramentPaleta.valorMaxim!=="undefined" && typeof estil_o_atrib.component[0].estiramentPaleta.valorMinim!=="undefined")
				cdns.push(DonaCadenaLang({"cat":"Interval de valors recomenats", "spa":"Intervaluo de valores recomendados", "eng":"Interval of recommended values", "fre":"Intervalle des valeurs recommand�es"}), ": [", estil_o_atrib.component[0].estiramentPaleta.valorMinim, ",", estil_o_atrib.component[0].estiramentPaleta.valorMaxim, "]");
		}
	}
	cdns.push("</div>");
	if (nc.n_capa>1)
...
{
	return DonaCadena(ParamCtrl.capa[i_capa].estil[(i_estil!=null) ? i_estil : parseInt(document.getElementById(prefix_id+"-estil").value)].desc) + " (" + DonaCadenaLang({"cat":"Reclassificaci�", "spa":"Reclasificaci�n", "eng":"Reclassification", "fre":"Reclassement"}) + ")";
}
...
function DonaNomNouEstilSeleccioCondicional(prefix_id, i_capa, i_estil)
{
	...
	return (DonaCadena(estil.desc)? DonaCadena(estil.desc): (estil.nom? estil.nom:DonaCadenaLang({"cat":"per defecte","spa":"por defecto","eng":"by default", "fre":"par d�faut"}) ))+
			" (" + DonaCadenaLang({"cat":"Selecci�", "spa":"Selecci�n", "eng":"Selection", "fre":"S�lection"}) + ")";
}
...
var cdns=[], consulta, nexe, capa, primer_i_estil_valid=null;
	...
	cdns.push("<div style=\"position:absolute; left:10px; top:10px;\">",
			DonaCadenaLang({"cat":"Mostra nom�s els valors de la capa", "spa":"Muestra solo los valores de la capa",
	...
	if (capa.estil && capa.estil.length)
	{
		...
		if (capa.estil.length>1)
		{
			...
			cdns.push("<input type=\"hidden\" value=\"0\" id=\"", prefix_id, "-estil\" name=\"estil\" />", DonaCadena(capa.estil[0].desc)?DonaCadena(capa.estil[0].desc): (capa.estil[0].nom?capa.estil[0].nom: DonaCadenaLang({"cat":"per defecte","spa":"por defecto","eng":"by default", "fre":"par d�faut"})));
		cdns.push("<br>");
	}
	cdns.push(DonaCadenaLang({"cat":"que cumplexien les condicions seg�ents", "spa":"que cumplen las condiciones seguientes",
	...
	for (var i_condicio=0; i_condicio<10; i_condicio++)
	{
		cdns.push("<span id=\"", prefix_id, "-nexe-", i_condicio, "\" class=\"Verdana11px\" style=\"display: "+((i_condicio==0) ? "inline" : "none")+"\"><fieldset><legend>",
			DonaCadenaLang({"cat":"Condici�", "spa":"Condici�n", "eng":"Condition", "fre":"Condition"}), " ", i_condicio+1, ":</legend>",
		...
		if (i_condicio<(10-1))
		{
			...
			cdns.push(DonaCadenaLang({"cat":"Nexe amb la seg�ent condici�", "spa":"Nexo con la siguiente condici�n", "eng":"Nexus with next condition", "fre":"Nexus avec la prochaine condition"}), ":",
...
{
var elem=ObreFinestra(window, "seleccioCondicional", DonaCadenaLang({"cat":"de selecci� per condici�",
...
var sel_condicional, i_estil_nou, estil, calcul, capa;
	...
	if(capa.estil.length==1 && !capa.estil[0].nom && !capa.estil[0].desc)
	{
		...
		capa.estil[0].desc={"cat":"per defecte","spa":"por defecto","eng":"by default", "fre":"par d�faut"};
	}
...
{
var elem=ObreFinestra(window, "combinacioRGB", DonaCadenaLang({"cat":"de combinaci� RGB",
...
var cdns=[], capa=ParamCtrl.capa[i_capa];
	...
	cdns.push("<div id=\"LayerCombinacioRGB\" style=\"position:absolute;left:10px;top:10px;\">",
			DonaCadenaLang({"cat":"Sel�lecciona les 3 components de la capa", "spa":"Selecciona las 3 componentes de la capa", "eng":"Select the three components of the layer", "fre":"S�lectionnez les trois composants de la couche"}), " \"",
	...
	cdns.push("<hr>",
		DonaCadenaLang({"cat":"La combinaci� RGB ser� afegida com a un estil nou de nom", "spa":"La combinaci�n RGB ser� a�adida como un estilo nuevo de nombre", "eng":"The RGB combination will be added as a new style with name", "fre":"La combinaison RVB sera ajout� en tant que nouveau style avec le nom"}),
		" <input type=\"text\" name=\"nom_estil\" class=\"Verdana11px\" style=\"width:400px;\" value=\"",
...
{
	return DonaCadenaLang({"cat":"Combinaci� RGB", "spa":"Combinaci�n RGB", "eng":"RGB combination", "fre":"Combinaison RVB"});
}
...
var cdns=[], capa=ParamCtrl.capa[i_capa], estil=capa.estil[i_estil];
	...
	if (!estil.histograma)
	{
		alert(DonaCadenaLang({"cat":"No es pot editar un estil no visualitzat", "spa":"No es puede editar un estilo no visualizado", "eng":"You cannot edit a style never visualized", "fre":"Vous ne pouvez pas �diter un style jamais visualis�"}));
		return "";
	}
	...
	if (estil.component)
	{
		cdns.push("<fieldset><legend>",
			DonaCadenaLang({"cat":"Valors per l'estirament de color", "spa":"Valores para el estiramiento de color", "eng":"Value for stretching of color", "fre":"Valeur pour l'�tirement de la couleur"}));
			cdns.push(":</legend>");
		for (var i_c=0; i_c<estil.component.length; i_c++)
		{
			...
			cdns.push("<label for=\"edita-estil-capa-valor-minim-", i_c, "\">", DonaCadenaLang({"cat":"M�nim", "spa":"M�nimo", "eng":"Minimum", "fre":"Minimum"}), ": </label>",
				...
				" (", DonaCadenaLang({"cat":"calculat", "spa":"calculado", "eng":"computed", "fre":"calcul�"}), " ", estil.histograma.component[i_c].valorMinimReal, " ",
				...
				"<br>");
			cdns.push("<label for=\"edita-estil-capa-valor-maxim-", i_c, "\">", DonaCadenaLang({"cat":"M�xim", "spa":"M�ximo", "eng":"Maximum", "fre":"Maximum"}), ": </label>",
				...
				" (", DonaCadenaLang({"cat":"calculat", "spa":"calculado", "eng":"computed", "fre":"calcul�"}), " ", estil.histograma.component[i_c].valorMaximReal, " ",
				"<input type=\"button\" class=\"Verdana11px\" value=\"", DonaCadenaLang({"cat":"Adoptar", "spa":"Adoptar", "eng":"Adopt", "fre":"Adopter"}),
...
{
var capa=ParamCtrl.capa[i_capa];
var elem=ObreFinestra(window, "mostraQualitat", DonaCadenaLang({"cat":"de mostrar la informaci� de qualitat",
						  "spa":"de mostrar la informaci�n de calidad",
						  "eng":"for showing the quality information",
						  "fre":"pour montrer l'information de qualit�"}));
	if (!elem)
		return;
